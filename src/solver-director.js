    ...
    worker.onmessage = ({data}) => {
      ...
      if (data.type === "done") {
        ...
        appendSearchLog("worker", `Finished ${plan.label}`, {
          ...
          arenaStates: data.arenaStates?.toLocaleString(),
          compactArenaBytes: data.compactArenaBytes,
          compactArenaAllocatedBytes: data.compactArenaAllocatedBytes,
          compactPathBytes: data.compactPathBytes,
          staleActions: data.staleActions,
          heapCompactions: data.heapCompactions,
          compactions: data.compactions,
        ...
        if (plan.handoffStage === "rewrite" && !options.resumeImprovement) {
          launchSideWorkers();
        }
