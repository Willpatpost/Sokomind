...
function collapseTunnelPushes(first, board, limit = 32, options = {}) {
  let state = {robot: first.robot, boxes: first.boxes};
  const path = [...first.path];
  let pushes = 1;
  let boxPosition = first.pushedTo;
  const seen = new Set([exactPushKey(state, board)]);
  while (pushes < limit && !goal(state.boxes, board.goals)) {
    const reachable = reachablePaths(state, board);
    if (createsSealedCorralDeadlock(state, board, reachable)) return null;
    const choices = pushBoxNeighbors(
      state, board, boxPosition, reachable, {lockProven: options.lockProven},
    );
    if (choices.length !== 1) break;
    const next = choices[0], signature = exactPushKey(next, board);
    if (seen.has(signature)) break;
    ...
    path.push(...next.path);
    state = {robot: next.robot, boxes: next.boxes};
    boxPosition = next.pushedTo;
    pushes++;
  }
  if (pushes === 1) return {...first, pushes: 1};
  return {
    ...state, path, pushes, pushClass: first.pushClass,
    pushedFrom: first.pushedFrom, pushedTo: boxPosition,
  };
}
...
function expandPushMacro(next, board, enabled = true, options = {}) {
  ...
  return collapseTunnelPushes(next, board, 32, options);
}
