...
// Batched, incremental search log renderer.
// Full renders use searchLog.slice(-1500) for the visible tail window.
// Appends are batched via requestAnimationFrame and rendered incrementally.
let _searchLogRafPending = false;
let _searchLogRenderedCount = 0;
const SEARCH_LOG_MAX_DOM_ENTRIES = 200;
...
function renderSearchLog() {
  ...
  _searchLogRenderedCount = searchLog.length;
}
function _scheduleIncrementalRender() {
  if (_searchLogRafPending) return;
  _searchLogRafPending = true;
  requestAnimationFrame(_flushSearchLog);
}
function _flushSearchLog() {
  _searchLogRafPending = false;
  $("search-log-count").textContent = searchLog.length.toLocaleString();
  const output = $("search-log-text");
  if (searchLog.length === 0) {
    output.value = "";
    _searchLogRenderedCount = 0;
    return;
  }
  // Incremental: append only new entries since last render
  const newEntries = searchLog.slice(_searchLogRenderedCount);
  if (newEntries.length > 0) {
    const newText = searchLogText(newEntries);
    const combined = output.value + (output.value ? "\n" : "") + newText;
    // Virtual scrolling: only keep last SEARCH_LOG_MAX_DOM_ENTRIES lines in DOM
    const lines = combined.split("\n");
    if (lines.length > SEARCH_LOG_MAX_DOM_ENTRIES) {
      output.value = lines.slice(lines.length - SEARCH_LOG_MAX_DOM_ENTRIES).join("\n");
    } else {
...
function title(key) {
  ...
  return key.split("-").map(x => x[0].toUpperCase() + x.slice(1)).join(" ");
}
...
function levelCard(key, rows) {
  ...
  size.textContent = puzzle
    ? `${rows[0].length}×${rows.length} · ${puzzle.boxes} box${puzzle.boxes !== 1 ? "es" : ""}`
