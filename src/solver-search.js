...
function beamSearch(payload) {
  ...
  const congestionWeight = payload.congestionWeight ?? 0.3;
  ...
  const chokepointData = congestionWeight
    ? identifyChokepoints(board) : null;
    ...
    for (const current of beam) {
      ...
      for (const rawNext of pushNeighbors(
        ...
        const evacuation = evacuationWeight
          ...
          : 0;
        const congestion = chokepointData
          ? congestionPenalty(child.boxes, chokepointData) : 0;
