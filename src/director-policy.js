(function attachDirectorPolicy(root, factory) {
  ...
  function exactTranspositionLimit(deviceMemory = 4, shardCount = 1) {
    ...
    const totalBudget = memory >= 32 ? 4000000
                      : memory >= 16 ? 2000000
                      : memory >= 8  ? 1200000
                      : memory >= 4  ? 640000
                      : memory >= 2  ? 320000
                      : 160000;
    return Math.max(80000, Math.min(1200000, Math.floor(totalBudget / shards)));
  }
  ...
  function portfolioWorkerCapacity(hardwareConcurrency = 2, deviceMemory = 4) {
    ...
    const memory = Number.isFinite(deviceMemory) && deviceMemory > 0 ? deviceMemory : 4;
    const memoryCapacity = memory <= 2 ? 2
                         : memory <= 4 ? 4
                         : memory <= 8 ? 8
                         : memory <= 16 ? 12
                         : 16;
    return Math.max(2, Math.min(hardware - 1, memoryCapacity));
  }
