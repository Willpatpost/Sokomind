...
describe("ProceduralAudioController", () => {
  ...
  describe("music generator", () => {
    ...
    it("disabling music via setPreferences stops the scheduler", async () => {
      ...
      const oscCountBefore = mockCtx._createdOscillators.length;
      controller.setPreferences(defaultPreferences({ soundEnabled: true, musicEnabled: false }));

      // Wait and verify no new oscillators are created
      await new Promise((r) => setTimeout(r, 200));
      // Should not have created significantly more oscillators
      // (the music scheduler should be stopped)
      controller.dispose();
    });
  });
