...
} = require("../../src/director-policy.js");
...
test("exact transposition capacity grows for a lone shard without unbounded multi-shard memory", () => {
  assert.equal(exactTranspositionLimit(8, 1), 1200000);
  assert.equal(exactTranspositionLimit(8, 3), 400000);
  assert.equal(exactTranspositionLimit(4, 3), 213333);
  assert.equal(exactTranspositionLimit(2, 1), 320000);
  assert.equal(exactTranspositionLimit(undefined, 1), 640000);
  assert.equal(exactTranspositionLimit(32, 1), 1200000);
  assert.equal(exactTranspositionLimit(32, 4), 1000000);
});
...
test("portfolio capacity respects both hardware and coarse memory pressure", () => {
  assert.equal(portfolioWorkerCapacity(16, 8), 8);
  assert.equal(portfolioWorkerCapacity(16, 4), 4);
  assert.equal(portfolioWorkerCapacity(16, 2), 2);
  assert.equal(portfolioWorkerCapacity(2, 8), 2);
  assert.equal(portfolioWorkerCapacity(24, 32), 16);
  assert.equal(portfolioWorkerCapacity(24, 16), 12);
});
